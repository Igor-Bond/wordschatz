...
`;
    ...
    тест('видовая пара глагола — одно слово, а не расхождение', () => {
        // Самая частая ложная тревога в русском: карточка учит «убирать»,
        // словарь знает «убрать». На настоящем наборе из двадцати слов
        // такая пара дала треть всех подозрений
        проверить.равно(wiktionary.matchTranslation('убирать', ['убрать', 'покончить']), 'match');
        проверить.равно(wiktionary.matchTranslation('показывать', ['показать']), 'match');
        проверить.равно(wiktionary.matchTranslation('рассказывать', ['рассказать']), 'match');
        проверить.равно(wiktionary.matchTranslation('собирать', ['собрать']), 'match');
    });

    тест('правило вида не склеивает разные слова', () => {
        // «стол» и «стул» отличаются одной буквой, но это разные вещи:
        // под правило вставки «ыва/ива/и» они не подпадают
        проверить.равно(wiktionary.matchTranslation('стул', ['стол']), 'differs');
        проверить.равно(wiktionary.matchTranslation('дом', ['дым']), 'differs');
    });
