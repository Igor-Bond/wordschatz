    ...
    fillAbout: async () => {
        ...
        if (screen) {
            ...
            if (метрики) {
                ...
                const сдвигМеню = document.documentElement.style
                    .getPropertyValue('--nav-drop') || '0px';

                // Главное число теперь это: сколько от подписей до края
                // самого дисплея. «Под кнопками» считается от края окна,
                // а окно, как выяснилось, до дисплея доходит не всегда —
                // ровно на этом я и просчитался четыре раза подряд
                const доКрая = (высотаЭкрана && первая)
                    ? Math.round(высотаЭкрана - верхОкна - первая.getBoundingClientRect().bottom)
                    : '—';

                метрики.textContent = `dvh ${dvh} · окно ${window.innerHeight}`
                    + ` · экран ${высотаЭкрана || '—'} · верх ${верхОкна}`
                    + ` · зона ${снизу} · меню ${низМеню} · под окном ${подОкном}`
                    + ` · видимое ${видимое}+${сдвиг} · опущено ${сдвигМеню}`
                    + ` · под кнопками ${подКнопками} · ДО КРАЯ ${доКрая}`;

                /*
                 * Приговор словами, а не числами.
                 *
                 * Пять заходов ушло на то, чтобы понять: меню стоит на
                 * краю окна, а окно не доходит до края экрана. Числа это
                 * показывали, но их надо было уметь прочесть. Пусть
                 * приложение читает их само.
                 *
                 * Порог в 8 точек — чтобы не срабатывать на округлениях
                 * и мелких расхождениях, которые ни на что не влияют.
                 */
                const строкаОбрезки = document.getElementById('about-cutoff');
                const рядОбрезки = document.getElementById('about-cutoff-row');
                if (строкаОбрезки && рядОбрезки) {
                    const обрезано = install.isStandalone()
                        && Number.isFinite(подОкном) && подОкном > 8;

                    рядОбрезки.classList.toggle('hidden', !обрезано);
                    if (обрезано) {
                        строкаОбрезки.textContent = t('settings.screenCutOff', { gap: подОкном });
                    }
                }
            }
        }
