...
/**
 * Имя базы можно подменить до первого импорта модуля.
 *
 * Нужно ровно одному потребителю — сквозным проверкам: они прогоняют
 * настоящий путь от слова до плана и обязаны делать это в стороне от
 * словаря живого человека. Приложение эту переменную не задаёт никогда,
 * поэтому у него всегда своя.
 */
const DB_NAME = globalThis.__WORTSCHATZ_DB__ || 'WortSchatzProDB';
...
export const dbService = {
    ...
    /**
     * Изменить запись пользователя, прочитав её под той же транзакцией.
     *
     * Запись одна на всё: опыт, лига, серия дней, дата последнего занятия
     * и настройки профиля. Пишут в неё из разных мест и почти
     * одновременно — в конце урока начисление опыта и запись серии
     * приходят с разницей в миллисекунды.
     *
     * Обычная последовательность «прочитал getUser, изменил, положил
     * обратно» в такой обстановке теряет чужие правки: кто прочитал
     * раньше, а записал позже, тот и затрёт. Именно это и случилось с
     * серией дней — она пропадала, а начисленный в том же уроке опыт
     * оставался на месте, потому что его запись легла последней.
     *
     * Здесь чтение и запись идут внутри одной транзакции, поэтому между
     * ними ничего не вклинится. Тем же способом уже защищены addXP и
     * saveStoredProfile — эти две функции просто написали раньше.
     *
     * @param {Object|Function} изменить поля или (текущий) => поля.
     *        Вернув null, ничего не пишем.
     * @returns {Promise<Object>} запись после изменения
     */
    updateUser: async (изменить) => {
        return await db.transaction('rw', db.user, async () => {
            const текущий = { ...DEFAULT_USER, ...(await db.user.get(1)), id: 1 };
            const изменения = typeof изменить === 'function' ? изменить(текущий) : изменить;

            if (!изменения) return текущий;

            const итог = dbService._stamp({ ...текущий, ...изменения, id: 1 });
            await db.user.put(итог);
            return итог;
        });
    },
