...
export const scheduler = {
    ...
    explainStreak: async () => {
        ...
        /*
         * Предложение восстановить серию по журналу занятий.
         *
         * Появляется, только когда журнал показывает больше, чем стоит в
         * счётчике. Такое расхождение означает, что счётчик потерялся:
         * ровно это и случилось из-за одновременной записи опыта и серии
         * в одну запись базы.
         *
         * Само собой это не применяется. Журнал знает, что человек в тот
         * день занимался, но не знает, довёл ли урок до конца, — а серия
         * растёт именно за доведённый. Поэтому решение остаётся за
         * человеком, и в вопросе честно сказано, откуда взялось число.
         */
        const поЖурналу = await scheduler.streakFromActivity();

        if (поЖурналу > value) {
            строки.push('', t('streak.foundInLog', { days: plural('common.day', поЖурналу) }));

            const восстановить = await dialog.confirm(строки.join('\n'), {
                title: t('streak.title'),
                okLabel: t('streak.restore'),
                cancelLabel: t('common.close')
            });

            if (восстановить) {
                await scheduler.restoreStreak(поЖурналу);
                await dialog.alert(t('streak.restored', { days: plural('common.day', поЖурналу) }),
                    { title: t('streak.title') });
            }
            return;
        }
    ...
    /**
     * Сколько дней подряд человек занимался — по журналу активности.
     *
     * Журнал ведётся отдельно от счётчика серии: на каждый день пишутся
     * опыт и число карточек — повторений и новых слов. Счётчики карточек
     * ставит только урок; свободная тренировка в Комнате и контроль темы
     * пишут один опыт. Поэтому «в этот день были карточки» — надёжный
     * признак учебного дня, не путающийся с разминкой.
     *
     * Считаем назад от сегодня. Сегодняшний день может быть ещё пустым —
     * это не разрыв, серия жива до конца суток; поэтому если сегодня
     * пусто, отсчёт начинается со вчера. Первый пустой день дальше
     * обрывает счёт.
     *
     * Чего этот способ не знает: доведён ли урок до конца. Карточки
     * могли быть оценены и в брошенном уроке. Поэтому число отсюда — не
     * истина, а предложение, и применяется оно только по кнопке.
     */
    streakFromActivity: async () => {
        const журнал = await dbService.getActivity(60);
        const учебный = (д) => (д.reviews || 0) + (д.newWords || 0) > 0;

        // От сегодняшнего дня к прошлому
        const дни = [...журнал].reverse();
        if (!дни.length) return 0;

        let начало = 0;
        if (!учебный(дни[0])) начало = 1;   // сегодня ещё не занимались — не разрыв

        let счёт = 0;
        for (let i = начало; i < дни.length; i++) {
            if (!учебный(дни[i])) break;
            счёт++;
        }
        return счёт;
    },

    /** Применить восстановленную серию: и счётчик, и дату последнего дня. */
    restoreStreak: async (дней) => {
        const журнал = await dbService.getActivity(60);
        const последний = [...журнал].reverse()
            .find(д => (д.reviews || 0) + (д.newWords || 0) > 0);

        await dbService.updateUser({
            currentStreak: дней,
            lastActiveDate: последний ? последний.date : dateUtils.today()
        });

        scheduler._paintStreak(дней);
        return дней;
    },
