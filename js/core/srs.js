...
export const srs = {
    ...
    calculate: (quality, word = {}) => {
        ...
        let next = srs.reviewOptions(interval, ease)[quality];
     ...
     */
    /**
     * Три срока повторения разом: «Трудно», «Хорошо», «Легко».
     *
     * Считаются вместе, потому что порознь они схлопывались. Формулы
     * разные — ×1,2, ×лёгкость и ×лёгкость×1,3, — но после округления до
     * целых дней при малом интервале и просевшей лёгкости они давали
     * одно и то же число. При интервале 1 и лёгкости 1,3 «Трудно» и
     * «Хорошо» обе обещали день, и обе честно: сроки действительно
     * совпадали. Владелец увидел это на кнопках — «одинаковое время на
     * плашках разного цвета».
     *
     * Замер до правки: восемнадцать совпадений на тысячу четыреста
     * сочетаний, все при интервале до восьми дней.
     *
     * Хуже того, при лёгкости у самого дна «Хорошо» не удлиняло интервал
     * вовсе: 1 × 1,3 = 1,3, округляется обратно в 1. Слово могло
     * возвращаться каждый день, сколько ни отвечай верно, — пока
     * лёгкость медленно не отрастёт.
     *
     * Поэтому здесь не просто формулы, а порядок: каждый следующий срок
     * хотя бы на день дальше предыдущего. Сдвигается только вверх и
     * только когда формула дала слишком мало, — там, где числа и так
     * расходятся, всё остаётся как было.
     *
     * @param {number} interval нынешний интервал в днях
     * @param {number} ease нынешняя лёгкость, до изменения
     * @returns {{2: number, 3: number, 4: number}} дни по оценкам
     */
    reviewOptions: (interval, ease) => {
        const трудно = Math.max(1, Math.round(interval * 1.2));
        const хорошо = Math.max(Math.round(interval * ease), трудно + 1);

        // «Легко» считается от уже поднятой лёгкости — так было и раньше
        const легко = Math.max(
            Math.round(interval * srs.clampEase(ease + 0.15) * 1.3),
            хорошо + 1
        );

        return { 2: трудно, 3: хорошо, 4: легко };
    },

    /**
     * Ошибка на контроле темы: слово возвращается в повторение завтра.
     *
     * До этого контроль был термометром: он замечал забытое слово —
     * освоенность падала, — но срок повторения не трогал вовсе. Слово с
     * интервалом в три недели, проваленное на контроле, возвращалось всё
     * равно через три недели. Приложение знало, что человек его не помнит,
     * и три недели делало вид, что помнит.
     *
     * Считается как обычный провал: интервал вдвое, лёгкость вниз, слово
     * назад в заучивание. Отличается только срок. Обычный провал зовёт
     * через десять минут, потому что случается посреди урока и человек
     * тут же продолжает; контроль на этом заканчивается, и звать в тот
     * же день некуда. Завтрашний день — ближайший, когда это осмысленно.
     *
     * repetitions не увеличивается: контроль — не повторение, и по этому
     * счётчику выбирается ступень заданий.
     */
    applyExamLapse: (word) => {
        const next = srs.calculate(1, word);

        word.interval = next.interval;
        word.ease = next.ease;
        word.phase = next.phase;
        word.stepIndex = next.stepIndex;
        word.nextReview = Date.now() + srs.DAY;
