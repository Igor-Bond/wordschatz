...
export const declension = {
    ...
    /**
     * Форма без основы — одно окончание.
     *
     * Нужно таблице: полные формы длинного прилагательного в пять
     * колонок на телефоне не помещаются и обрезаются ровно по тому
     * месту, ради которого таблицу и открывают, — по окончанию.
     * «interessante» в клетке шириной 57 точек требует 63.
     *
     * Отрезается именно stem, а не общая часть всех форм. Общая часть
     * подвела бы: у «interessant» все тридцать шесть форм начинаются с
     * «interessante», и окончания вышли бы «-», «-n», «-r», «-s» вместо
     * настоящих «-e», «-en», «-er», «-es».
     */
    ending: (adjective, form) => {
        const основа = declension.stem(adjective);
        const текст = String(form ?? '');

        if (!основа || !текст.startsWith(основа)) return текст;
        return текст.slice(основа.length);
    },
