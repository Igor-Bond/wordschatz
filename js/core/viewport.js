...
export const viewport = {
     ...
     *
     * Опускаем только туда, где рисовать можно. Настоящее лечение —
     * метка строки состояния `black` в index.html, но её iOS запоминает
     * при установке значка, и уже поставленному приложению она не
     * поможет. Здесь же всё чинится само: как только окно дойдёт до
     * низа, разница станет нулём и сдвиг снимется.
     */
    pinNav: () => {
        const корень = document.documentElement;
        const { зазор, наше } = viewport.measureBottom();
