const scanner = {
    ...
    render: () => {
        ...
        main.innerHTML = `
            <div class="fade-in max-w-lg mx-auto mt-2 pb-10">
                <h2 class="text-2xl font-bold text-slate-100 mb-6 text-center">Добавление слов</h2>
                
                <!-- Вкладки (Режимы) -->
                <div class="flex bg-slate-800 rounded-xl p-1 mb-6 border border-slate-700">
                    <button onclick="scanner.switchTab('single')" id="tab-single" class="flex-1 py-2 text-sm font-bold rounded-lg bg-amber-500 text-slate-900 shadow transition-all">Одно слово</button>
                    <button onclick="scanner.switchTab('topic')" id="tab-topic" class="flex-1 py-2 text-sm font-bold rounded-lg text-slate-400 hover:text-slate-200 transition-all">Тема целиком</button>
                    <button onclick="scanner.switchTab('text')" id="tab-text" class="flex-1 py-2 text-sm font-bold rounded-lg text-slate-400 hover:text-slate-200 transition-all">Текст</button>
                </div>

                <!-- Режим: Одно слово -->
                <div id="mode-single" class="space-y-4">
                    <div class="bg-slate-800 p-5 rounded-2xl border border-slate-700 shadow-lg">
                        <label class="block text-sm font-bold text-slate-400 mb-2">Немецкое слово или фраза</label>
                        <div class="flex gap-2">
                            <input type="text" id="scan-word-input" class="flex-1 bg-slate-900 border-2 border-slate-600 text-slate-100 rounded-xl px-4 py-3 outline-none focus:border-amber-500 transition-colors" placeholder="Например: Leistung...">
                ...
                <div id="mode-topic" class="space-y-4 hidden">
                    <div class="bg-slate-800 p-5 rounded-2xl border border-slate-700 shadow-lg">
                        <label class="block text-sm font-bold text-slate-400 mb-2">Какую тему сгенерировать?</label>
                        <input type="text" id="scan-topic-input" class="w-full bg-slate-900 border-2 border-slate-600 text-slate-100 rounded-xl px-4 py-3 mb-4 outline-none focus:border-amber-500 transition-colors" placeholder="Еда, Путешествия, Схемотехника...">
                        
                        <label class="block text-sm font-bold text-slate-400 mb-2">Количество слов (до 30)</label>
                ...
                <div id="mode-text" class="space-y-4 hidden">
                    <div class="bg-slate-800 p-5 rounded-2xl border border-slate-700 shadow-lg">
                        <label class="block text-sm font-bold text-slate-400 mb-2">Текст для анализа</label>
                        <textarea id="scan-text-input" class="w-full h-32 bg-slate-900 border-2 border-slate-600 text-slate-100 rounded-xl px-4 py-3 mb-6 outline-none focus:border-amber-500 transition-colors resize-none" placeholder="Вставьте немецкий текст сюда..."></textarea>
                        ...
                        <button onclick="scanner.analyzeText()" class="w-full py-4 bg-amber-500 text-slate-900 text-lg font-black rounded-xl shadow-lg hover:bg-amber-400 transition-transform active:scale-95 flex items-center justify-center gap-2">
                            <i class="fa-solid fa-magnifying-glass-text"></i> ПРОАНАЛИЗИРОВАТЬ
                        </button>
                    </div>
                </div>
    ...
    searchWord: async () => {
        ...
        if (!input) return;
        scanner.showLoader(`Анализируем слово "${input}"...`);
...
${aiService._getJsonFormat()}`;
            ...
            let result = aiService._parseJsonResponse(rawResponse, config.getProfile().level, 'Разное');
    ...
    generateTopic: async () => {
        const topic = document.getElementById('scan-topic-input').value.trim() || 'Повседневная жизнь';
        const count = document.getElementById('scan-count-input').value;
        scanner.showLoader(`Нейросеть собирает ${count} слов на тему "${topic}"...`);
    ...
    analyzeText: async () => {
        ...
        if (!text) return;
        scanner.showLoader(`Анализируем текст, ищем полезные слова...`);
...
${aiService._getJsonFormat()}`;
            ...
            const rawResponse = await aiService.callGemini(prompt, true);
            let result = aiService._parseJsonResponse(rawResponse, profile.level, 'Из текста');
    ...
    renderResults: (wordsArray) => {
        ...
        if (!wordsArray || wordsArray.length === 0) {
            container.innerHTML = `<p class="text-center text-slate-500">Ничего не найдено.</p>`;
            return;
        }
        ...
        let html = `
            <div class="flex justify-between items-center mb-4 px-2 py-3 bg-slate-900/90 backdrop-blur-md sticky top-[72px] z-20 border-b border-slate-700 rounded-xl shadow-lg">
                <span class="text-sm font-bold text-slate-300">Выбрано: <span id="scan-selected-count" class="text-amber-500 text-lg">${selectedCount}</span> / ${wordsArray.length}</span>
    ...
    saveSelected: async () => {
        ...
        document.getElementById('scan-results').innerHTML = `
            <div class="flex flex-col items-center justify-center py-10 fade-in text-center bg-slate-800 rounded-xl border border-slate-700">
                ...
                <h3 class="text-2xl font-bold text-slate-100 mb-2">Успешно сохранено!</h3>
                <p class="text-slate-400 text-lg">Добавлено новых слов: <span class="text-white font-bold">${count}</span></p>
