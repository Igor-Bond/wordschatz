...
export const chat = {
    ...
    sendMessage: async () => {
        ...
        try {
            ...
            /*
             * Раньше любая неудача подавалась как «ошибка сети», а
             * настоящая причина пряталась в скобках третьим уровнем.
             * Первое, что читал человек с недействительным ключом, —
             * что виновата сеть. Своё сообщение оставляем только там,
             * где его нет: у обрыва связи текст обычно нечитаемый.
             */
            const причина = String(error?.message || '').trim();
            chat.renderMessage('ai', причина || t('chat.networkError'));
