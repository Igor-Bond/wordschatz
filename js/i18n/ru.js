export const ru = {
    ...
    goals: {
        5: '~ 15 мин в день (Лёгкий старт)',
        10: '~ 27 мин в день (Стандарт)',
        15: '~ 40 мин в день (Интенсив)',
        20: '~ 55 мин в день (Хардкор)'
    },

    onboarding: {
        appName: 'WortSchatz Pro',
        intro: 'Умная система изучения немецкого языка с ИИ. Давай настроим твой личный план обучения.',
        guideHint: 'Разбор приложения — что где лежит и в каком порядке этим пользоваться — там же, в «Профиле»: кнопка «Как пользоваться приложением».',
    ...
    dashboard: {
        ...
        goalLowered: 'Норма новых слов снижена с {from} до {to} в день: повторения не помещались в урок три дня подряд. Так учится меньше слов, зато выученное не сыпется. Поднять обратно можно в настройках.',
        startLesson: 'НАЧАТЬ УРОК ({count})',
        estimate: 'Примерно {minutes} — по вашей скорости на прошлых уроках',
        estimateRough: 'Примерно {minutes}. После первого урока оценка станет по вашей скорости',
    ...
    cycle: {
        ...
        fromInterests: 'По вашим интересам',
        orAnother: 'Или другое',
    ...
    auth: {
        ...
        hint: 'Словарь и прогресс можно держать в облаке — тогда они откроются и на другом телефоне, и на компьютере. Настраивается это позже, в приложении.',
        whereToFind: 'Включить можно в любой момент: «Профиль» в нижнем меню — кнопка «Войти через Google».',
        worksOffline: 'Без входа приложение работает целиком: словарь, уроки и прогресс хранятся на устройстве. Облако нужно, только чтобы открыть их ещё и на другом телефоне или компьютере.',
    ...
    streak: {
        ...
        resetHint: 'Если цифра неверна. Отсчёт пойдёт заново с первого пройденного плана'
    },
    ...
    exercises: {
        ...
        buildWord: 'Соберите слово из букв',
        wordBuilderHint: 'Нажмите на букву, чтобы убрать её',
        ...
        builderRetry: 'Порядок не тот — уберите лишние слова и соберите заново.',
        builderFromTry: 'Собрано с {n}-й попытки · +{xp} XP',
    ...
    control: {
        ...
        returnedToReview: 'Эти слова вернутся в повторение завтра — {words}. Контроль показал, что они забыты, и ждать прежнего срока незачем.',
        mistake: 'Ошибка',
    ...
    profile: {
        ...
        tabStats: 'Статистика',
        tabDict: 'Мой словарь',
        guide: 'Как пользоваться приложением',
        defaultName: 'Студент',
